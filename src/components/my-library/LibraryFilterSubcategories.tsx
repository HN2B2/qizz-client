...
const LibraryFilterSubcategories = () => {
    ...
    const getData = async () => {
        try {
            const rawCategories: { data: Category[]; total: number } =
                await instance.get(`categories?limit=100`).json()
    ...
    const handleSelectCategory = (value: string | null) => {
        handlerSubCategories.setState(
            categories.find((item) => item.id === Number(value))
                ?.subCategories ?? []
        )
    }
            ...
            >
                <>
                    ...
                    <Group justify="space-between" mt="100">
                        ...
                        <Button onClick={() => handleSubmitSubcategories()}>
                            Confirm
                        </Button>
                    </Group>
                </>
